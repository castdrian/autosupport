...
export class UtteranceCommand extends Command {
	public override async contextMenuRun(
		...
		try {
			...
			const intents = await witIntents(config.devWitAiServerToken ?? config.witAiServerToken[interaction.targetMessage.guildId]);
			...
			try {
				...
				if (confirmation.customId === "select_intent") {
					...
					await trainWitUtterance(interaction.targetMessage.content, intent.name, config.devWitAiServerToken ?? config.witAiServerToken[interaction.targetMessage.guildId]);
